...
<script>
    ...
    export default {
        ...
        async onShow() {
            ...
            let sortModel = data.sortModel;
            let distanceValue = data.distanceValue;
            ...
            if (this.distanceOptions.length < 1) {
                ...
                let searchRange = res.data.data.searchRange;
                for(let i = 0; i < searchRange.length; ++i) {
                    this.distanceOptions.push({
                        text: searchRange[i] + "km",
                        value: searchRange[i]
                    })
                }
            }
            this.distanceValue = this.distanceOptions[0].value;

            this.distanceValue = distanceValue == '' ? this.distanceValue : distanceValue; 

            let res = await this.R.fetchSortModel();
            this.sortModelArr = res.data.data.sortModel;
            this.sortModel = this.sortModelArr[0].type;

            this.sortModel = sortModel == '' ? this.sortModel : sortModel;
            for(let i = 0; i < this.sortModelArr.length; ++i) {
                if (this.sortModelArr[i].type == this.sortModel) {
                    this.currentIndex = i;
                    break;
                }
            }
            ...
            this.goods = [];
            this.getGoods(this.distanceOptions[0].value);
        },
